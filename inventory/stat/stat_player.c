...
static void display_texts(stats_t *stats)
{
    return;
}

static void initialize_texts(stats_t *stats, csfml_t *page)
{
    ...
    stats->att_txt = make_text(page->font_itim, "ATTACK:", att_txt_pos, 10);
    stats->def_txt = make_text(page->font_itim, "DEFENSE:", def_txt_pos, 3);
    stats->speed_txt = make_text(page->font_itim, "SPEED:", spe_txt_pos, 10);
}

void initialize_graphical_stats(inventory_t *inventory, csfml_t *page)
{
    sfVector2f get_pl_pos;

    get_pl_pos = sfSprite_getPosition(page->player.player);
    get_pl_pos.x -= 595;
    get_pl_pos.y += 190;
    ...
    initialize_texts(&inventory->stats, page);
}
